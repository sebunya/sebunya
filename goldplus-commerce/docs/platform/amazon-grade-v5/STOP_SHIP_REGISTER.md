...
| 7.12 | Money types/constraints | **REPOSITORY-COMPLETE (verified) / PRODUCTION-PENDING** | *Repository:* migration `0062_money_bigint_and_checks` widens every monetary column int4→bigint and adds non-negative money + positive-quantity CHECKs (NOT VALID then VALIDATE to bound the lock window); drizzle schema widened; parity 63/63; proven on real PostgreSQL (clean apply, 9e9 UGX round-trip, negative money rejected, corrupt pre-existing row fails VALIDATE loudly). *Production deployment is a separate gate:* per §11, applying `0062` in production requires table-size measurement, ALTER lock/rewrite behaviour on a production-like clone, `lock_timeout`/`statement_timeout`, a data-violation scan, reconciliation SQL, an abort plan, and verified backup/restore evidence. Those are Lane B evidence items, NOT unfinished repository engineering. The distinction is deliberate: a green repository migration is not a proven production migration |
