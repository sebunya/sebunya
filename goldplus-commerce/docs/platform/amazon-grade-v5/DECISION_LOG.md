...
- **D-3D-1 · Conservative Zod boundary over a rip-and-replace.** Kept the existing env-specific assembly (prod required/strong, test safe defaults) and made a Zod `configSchema` the invariant every path must satisfy, with the type DERIVED via `z.infer`. Preserves the exported `Config`/`env` shape so no consumer or the migrate runner breaks, while gaining schema validation + `redactedConfig()`. The full `process.env`→`env` sweep is a mechanical follow-on. *Additive, low-risk.*
