...
| H-8 | Application `console.*` | 6 use-case/repo files (7 calls) + LocalProductImageStorage | **CORRECTED (Slice 11)**: routed through the structured `logger` with preserved context ({ err, ids }, message). Bootstrap (`config/env.ts`, `otel.ts`) and CLI migration scripts (`migrate.ts`, `rehearse.ts`) keep `console` legitimately (pre-logger / standalone tools). |
