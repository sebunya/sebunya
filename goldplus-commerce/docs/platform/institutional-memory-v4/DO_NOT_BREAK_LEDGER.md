...
| 23 | Web image rebuilds may serve stale page layers from docker cache; when a shipped page change does not appear live, verify IN_IMAGE and rebuild --no-cache before deeper debugging | W-USERMGMT incident | note in release recipe |
| 24 | A mid-render throw in Astro SSR streams a TRUNCATED 200 — status sweeps cannot catch it; check web container logs (ReferenceError/uncaught) after page-touching rolls | W-USERMGMT incident | consider a route-contract end-marker assertion |
| 25 | PLATFORM_ADMINISTRATOR is never granted in one step: request → different-admin decision (self-decide 403), requester-only withdraw, self-revoke lockout guard | governance suite 7/7 + live proofs | keep |
| 26 | Live customer messaging requires SEVEN aligned gates (4 governance + live-send + channel + empty allowlist) AND recipient config; each defaults false so silence never reads as permission — verified reachable end-to-end (ALLOW_LIVE) on 2026-08-04 | activation wave live proof | never assume a flag is honoured until a real send is attempted |
| 27 | Loyalty needs TWO keys: LOYALTY_PROGRAMME_ENABLED (deployment) + admin config enabled — either off means no points issued | activation wave | keep both; points are a monetary liability |
| 28 | A raw Set-Cookie header on Astro.response is DROPPED when frontmatter returns Astro.redirect() — always use Astro.cookies.set/delete for any cookie that must survive a redirect | checkout+account wave live proof | admin worked, customer flows silently broke |
| 29 | Every order INSERT must carry the verified checkout principal (orders.userId; GUEST → null) — account history and loyalty attribution both hang off it; the column existed 6+ months with nothing writing it | checkout+account wave | a schema column existing is not evidence anything writes it |
| 30 | Browser <script> bundles must NEVER import the '@goldplus/shared' package root — its index pulls node:crypto modules and the vite client build fails; deep-import pure-data modules instead | location wave build failure | SSR imports masking client-bundle breakage |
| 31 | Delivery districts are a CLOSED vocabulary (UGANDA_DISTRICTS + normalizeUgandaDistrict in shared): checkout and saved addresses canonicalise or refuse — never reintroduce free-text or unverified gazetteer districts | location wave | wrong districts silently mis-zone real deliveries |
| 32 | Delivery-fee estimates NEVER enter order money: only an enabled operator zone (source=ZONE) confirms a fee and joins totals; OBSERVED/MODEL are advisory copy only, and unknown geography is UNAVAILABLE, never a guess | delivery-intelligence wave live proofs (adopt→CONFIRMED→delete→ESTIMATED) | estimateDeliveryFee precedence unit-pinned (15 tests); keep checkout using deliveryFeeConfirmed only |
| 33 | delivery_pricing_policy is a SINGLETON keyed on 'policy' (unique index); readers fall back to DEFAULT_DELIVERY_BAND_POLICY when unstored — never seed the row in a migration, saving it is the operator owning the numbers | migration 0083 + SaveDeliveryPricingPolicyUseCase | validateBandPolicy + upsert onConflict target singleton |
| 34 | Anything serialized into HTML for the BROWSER (data-* attributes, define:vars inline scripts) must use `publicApiBase`, never runtime `apiBase` — SSR resolves apiBase to the compose-internal http://api:3000 and every client fetch dies unresolvable | shopping-assistant recovery (assistant down + shop autocomplete silently dead from the same leak) | client scripts prefer build-inlined PUBLIC_API_BASE_URL; audit any new data-api-* attribute |
