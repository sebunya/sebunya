...
| API 5xx / `*_FAILED` | 0 × 5xx in ~86k requests (37k 200, 11k 404 = empty-cart lookups + redirect-resolver misses, pre-existing); level-50 lines = 4, all Lighthouse-Watch shortfall reports; the level-40 lines are the AI-visibility sweeper's routine "mark dead runs" query (aiv_runs has 0 rows — no run was killed) | pass |
...
### 23.2 Loose ends from the host read (2026-09-21 07:10 UTC)
- **sGTM production container "unhealthy" for 37 h was a broken healthcheck, not a broken service.** Our compose override ran a bare `node`, which is not on the image's PATH → 4,464 consecutive failed checks, while `/healthy` on 127.0.0.1:8080 answered 200 throughout and usage beacons kept flowing. Fixed in `docker-compose.production.yml` by using the image's own checker (`/nodejs/bin/node /app/health_checker_bin.js`, the command the healthy preview container inherits); applied by recreating only `sgtm-production`. Server-side GA4 was never down; the false state would have hidden a real outage and blocks any `service_healthy` dependency.
- Caddy "certificate needs renewal" lines are ARI bookkeeping; renewal windows open in November 2026 — nothing due.
- Containment switch defaults verified in code: `PROFILE_READ_PURE !== 'false'`, `RESPONSE_EVENT_TO_METRIC === 'false'` to revert, `SSR_IDENTITY_V2 === 'false'` to revert — unset means ON, matching the observed behaviour.
- Brand-new visitor minting exactly one profile: still unseen (0 new profiles since 19:30 UTC as of 07:10).
