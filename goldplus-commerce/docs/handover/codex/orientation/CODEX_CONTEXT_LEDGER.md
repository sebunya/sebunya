...
- `recordProviderOutcome` increments attempts only for real calls, dead-letters the eighth known failure, releases a cap only for zero-attempt outcomes, and retains the original cap for live failures, dead letters, unknown outcomes, and replay. OUTCOME_UNKNOWN cannot enter the replay path and requires explicit actor/reason evidence to reconcile to SENT or FAILED.
