...
## Behavioral Interventions — ethical measured guidance

- Verified base: clean local/origin Copy Quality evidence head `fc3aa1e00093a38b3bbc921b006a61942445e2c7`. Reconciliation found Experiment, Customer DNA, consent and measurement foundations but no intervention bounded context. Additive migration `0046` is required; migrations `0000`–`0045` remain unchanged.
- Immutable versions bind target behavior, the authoritative Experiment hypothesis/metric, treatment variant, audience, ON_SITE placement, truthful dismissible content and bounded frequency/dismissal policy. Manipulative urgency/scarcity/coercion, external channels and transaction CTAs fail closed.
- Eligibility requires ACTIVE intervention, RUNNING Experiment, deterministic treatment assignment, current personalization consent and matching Customer DNA lifecycle. Exposure rechecks gates under an advisory transaction lock; Experiment assignment/exposure and intervention exposure commit atomically. Public APIs omit internal consent/identity/experiment evidence.
- Authenticated engagement/dismissal and server-measured target outcomes are idempotent and aggregate truthfully. Exact read/create/manage/approve/activate RBAC, four-eyes approval, protected API and Astro operator/customer surfaces are wired. No provider, outbox, Automation or customer-communication path exists.
- PostgreSQL proof passes control exclusion, consent recheck, one frequency winner, one atomic assignment/Experiment exposure/intervention exposure, retry/ownership/suppression/outcome truth, native JSONB, five events, zero protected deltas/provider calls/residue. Fresh `0000`–`0046` replay has 47 rows/five empty tables. Focused 61/61, typecheck/build/secret/changed-path lint/diff and clean 212-file / 4,102-test suite pass; repository lint retains only its unrelated baseline error.
- Source commit `42a3aa1ff82933eec4bab662aea789e3a505d6f3`; status `SOURCE_COMPLETE_NOT_DEPLOYED`. No production migration, live intervention or customer communication occurred. Next engineering queue item: Loyalty.

## Loyalty — auditable points liability ledger

- Verified base: clean local/origin Behavioral Interventions evidence head `fbda7f9b06a719b797d625aab17d2538fd7f0746`. Reconciliation found an existing dormant ledger but active concurrency, implicit-expiry, missing-expiry, weak collision and administrator-operability defects.
- Migration `0047` adds type/sign/source checks, related-entry integrity and unique expiry/reversal source constraints without changing migrations `0000`–`0046`. Balance is never stored: every liability change is the signed sum of immutable ledger events. FIFO allocation expires only the unspent remainder and only through an explicit expiry event.
- Per-account advisory transaction locks serialize ledger appends, redemption balance validation, expiry and reversal. Order earn, redemption, expiry and reversal retries converge; a reused key with different immutable facts fails closed. Account history is read-only and no longer creates empty accounts.
- Verified PesaPal completion loads paid customer identity and total from the persisted order and invokes the dual environment/config-gated earn path. The gate stays dormant. The settings-protected API/Astro control room exposes aggregate liability, due expiry and recent PII-minimized events; configuration, expiry and reversal are explicit audited operations.
- Real PostgreSQL proves dormant denial, duplicate paid-order convergence, one winner from two concurrent redemptions, exact 20-point partial expiry after 80 of 100 points were spent, idempotent reversal/expiry, collision and database-shape denial, no mutable balance, zero protected deltas/provider calls/residue. Fresh and populated migration paths reach 48 records with all constraints.
- Focused Loyalty/API/UI/architecture tests pass 99/99; typecheck/build/secret/changed-TypeScript lint/diff checks pass. Clean full suite passes 214 files / 4,116 tests. Repository lint retains only the unrelated `ICustomerDnaRepository.ts:6` baseline.
- Source commit `32e3ef0c24aa2bd06c85c73400b7dd2751507389`; status `SOURCE_COMPLETE_NOT_DEPLOYED`. No production migration, commercial activation, live ledger mutation or customer communication occurred. Next engineering queue item: Search Insights.

## Search Insights — privacy-safe aggregate analytics

- Verified base: clean local/origin Loyalty evidence head `c35d61723802c34edf8ea89bba353c3256ab6846`. Reconciliation found anonymous query/zero-result demand capture but no per-query/product CTR, add-to-cart conversion, observed-rank or synonym evidence boundary.
- Migration `0048` adds aggregate-only query/product counters with foreign keys and database constraints enforcing clicks not above impressions, conversions not above clicks and bounded ranks. Migrations `0000`–`0047` remain unchanged; no raw event history or visitor/session/customer/contact/commerce identifier is stored.
- Capture accepts only normalized bounded query, existing product UUID, rank and interaction type. Interaction without prior impression fails closed. Low-volume terms are suppressed from the protected read model; add-to-cart is the explicit conversion boundary, while ranking and synonym candidates remain evidence-only and create no catalogue rule.
- The reports-protected API and persisted Astro administrator surface expose totals, zero-result rate, CTR, add-to-cart rate, ranking and synonym evidence with truthful empty, unavailable and privacy states. Catalogue, checkout, order, payment, consent, provider and communication behavior remains outside the write boundary.
- Real PostgreSQL proves ten-way concurrent atomic aggregation, bounded click/conversion caps, low-volume suppression, evidence-only synonym derivation, zero raw-history linkage columns, zero protected deltas/provider calls/residue. Fresh `0000`–`0048` replay produces 49 migration rows, two empty Search tables and four constraints.
- Focused Search/API/UI/architecture/admin-protection tests pass 60/60; workspace typecheck/build, secret scan, changed-TypeScript lint and diff check pass. Clean full suite passes 216 files / 4,129 tests. Repository lint retains only the unrelated `ICustomerDnaRepository.ts:6` baseline.
- Source commit `682384b2a862e86ce3a14f4f5a875506f4a9d33f`; status `SOURCE_COMPLETE_NOT_DEPLOYED`. The engineering-controlled source queue is complete. No production migration, deployment, rule activation, provider action or live claim occurred.
