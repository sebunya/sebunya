...
9. LIVE VERIFIED after the roll (dfa50ef0, 4/4 healthy): an override written from inside the api container (budget LCP 2400, canary 20 s) was applied by the next queued run `20260913T075708Z` — the k6 canary ran 20 s and `state/config.effective.json` reports `admin_settings.applied` with the two sections; the settings page rendered every section with "overridden" badges on exactly those two and every credential as "not set". The test override was then reset to the defaults. The page's own Save button was not clicked by the assistant (form submission is the owner's action); the write it performs is the one verified.
