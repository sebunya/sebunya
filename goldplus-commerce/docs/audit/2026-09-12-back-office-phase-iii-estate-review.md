...
| Operational health (`/admin/system`, `/health`) | System administrator | 3 | OPERATED | Lighthouse Watch (2026-09-13): real Lighthouse every 6 h and after every deploy, stored as Web Vitals rows, scored against 100 per category, SEO alerts raised/resolved, Prometheus gauge, ALERT log line; shown on the Core Web Vitals screen | 3 | `LighthouseWatch` (11) | Synthetic catalogue monitor is blind (Cloudflare 403 to Node fetch, pre-existing) | READY, one blind monitor |
