...
0031 inventory ledger (reserved_quantity + reorder_point + inventory_reservations)
...
- Proofs: fresh 0000→0031 ✓ · pre-0023 production-shaped upgrade ✓ (153 ms,
...
- `/root/APPROVE_GOLDPLUS_DB_BACKUP_AND_MIGRATIONS_0023_0028`
  ...
  NOTE: the pending manifest now extends through **0031** (fulfilment 0029/0030 + inventory 0031).
  Confirm with the operator whether the same marker authorises 0023→0031, or issue a
  refreshed marker phrase `..._0023_0031`. Do not apply 0029/0030/0031 until confirmed.
...
→ 6 API image-start + web smokes → 7 run pre-flight query → 8 apply 0023→0031 →
9 verify ledger 32 rows / schema fingerprints / FK+orphan checks →
