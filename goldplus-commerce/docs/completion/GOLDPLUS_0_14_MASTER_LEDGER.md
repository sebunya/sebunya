...
| 3 | Checkout / location / payment | checkout.astro, checkout/pesapal/* | commerce, webhooks | StartCheckout, StartPesaPalPayment, VerifyPesaPalPayment, RecordPaymentWebhook | PaymentStateMachine, PaymentIdempotencyService | IPesaPalClient, IPesaPal/IPayment repos | DrizzlePayment{,Attempt} | commerce.ts | payments.confirm/read | yes | payment unit suites | LIVE (evidence) | PARTIAL_VERTICAL | Uganda location hierarchy, delivery fee rules, admin payment ops depth, reconciliation view | LOW — extend, never duplicate state machine | **next implementation slice** |
