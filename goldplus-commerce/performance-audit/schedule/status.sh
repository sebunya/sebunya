...
s=json.load(open(sys.argv[1])); now=datetime.datetime.utcnow().replace(microsecond=0)
...
echo "== latest run"; L="$(cat "$DATA_DIR/state/last_run_id" 2>/dev/null)"; [ -n "$L" ] && python3 -c 'import json,sys;m=json.load(open(sys.argv[1]));print(f"  {m[\"run_id\"]} kind={m[\"kind\"]} label={m.get(\"label\")} outcome={m.get(\"outcome\")} metrics={m.get(\"metric_count\")}")' "$DATA_DIR/reports/$L/manifest.json" || echo "  none"
