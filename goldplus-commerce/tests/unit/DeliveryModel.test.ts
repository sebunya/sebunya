...
describe('the six refusals', () => {
  ...
  it('refuses an unserviceable area at any price', () => {
    const r = quoteDelivery(inputs({ area: area({ serviceable: false }) }));
    expect(r).toMatchObject({ available: false, reason: 'AREA_UNSERVICEABLE' });
  });

  it('refuses a water area — pickup only, no road quote and no surcharge', () => {
    const r = quoteDelivery(inputs({ area: area({ accessMode: 'water' }) }));
  ...
  it('refuses an area outside the metro corridor set', () => {
    const r = quoteDelivery(inputs({ area: area({ corridor: null, band: null, accessMode: null }) }));
    expect(r).toMatchObject({ available: false, reason: 'AREA_NOT_METRO' });
  });

  it('distinguishes an upcountry area from an address that did not resolve', () => {
    // An Adjumani order resolved perfectly; we simply do not price upcountry
    // automatically. Telling that customer we could not find their address
    // would be false, and it belongs in a different ops queue.
    const upcountry = quoteDelivery(
      inputs({ area: area({ areaSlug: 'adjumani-esia-61004', corridor: null, band: null, accessMode: null }) }),
    );
    const unresolved = quoteDelivery(inputs({ area: null }));
    expect(upcountry).toMatchObject({ available: false, reason: 'AREA_NOT_METRO' });
    expect(unresolved).toMatchObject({ available: false, reason: 'AREA_UNRESOLVED' });
    // The upcountry refusal still knows which area it was.
    expect(upcountry.available === false && upcountry.explanation.areaSlug).toBe('adjumani-esia-61004');
  });

  it('reports AREA_NOT_METRO before serviceability, which is unknown upcountry', () => {
    // An area with no corridor row has no serviceability flag either, so
    // claiming it is unserviceable would be inventing a fact.
    const r = quoteDelivery(
      inputs({ area: area({ corridor: null, band: null, accessMode: null, serviceable: false }) }),
    );
    expect(r).toMatchObject({ available: false, reason: 'AREA_NOT_METRO' });
  });
  ...
  it('reports the customer-facing problem before our own', () => {
    ...
    const r = quoteDelivery(inputs({ area: area({ accessMode: 'water' }), config: {}, hasActiveOrigin: false }));
    expect(r).toMatchObject({ available: false, reason: 'WATER_ACCESS' });
  });
  ...
  it('can produce every one of the seven reasons', () => {
    ...
    produced.add((quoteDelivery(inputs({ hasActiveOrigin: false })) as any).reason);
    produced.add((quoteDelivery(inputs({ config: {} })) as any).reason);
    expect([...produced].sort()).toEqual([...UNAVAILABLE_REASONS].sort());
  });

  it('has a distinct copy key for every reason', () => {
    const keys = new Set(UNAVAILABLE_REASONS.map((r) => `copy_unavailable_${r.toLowerCase()}`));
    expect(keys.size).toBe(UNAVAILABLE_REASONS.length);
    for (const key of keys) {
      expect(DELIVERY_CONFIG_REGISTRY.some((e) => e.key === key)).toBe(true);
    }
  });
});
...
describe('the launch configuration', () => {
  it('has exactly five mandatory values, and a sixth that ships off', () => {
    const mandatory = DELIVERY_CONFIG_REGISTRY.filter((e) => e.mandatory).map((e) => e.key);
    expect(mandatory.sort()).toEqual([...LAUNCH_KEYS].sort());
...
describe('the band preview that catches a mistyped speed', () => {
  const samples = DISTANCE_BANDS.map((band) => ({
    ...
    area: area({ band, areaSlug: `area-${band}` }),
  }));
