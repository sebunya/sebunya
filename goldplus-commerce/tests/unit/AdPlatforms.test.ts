...
import { AD_PLATFORMS, buildAdRequest, hashEmail, hashPhone, normalisePhoneUg } from '../../apps/api/src/infrastructure/advertising/AdPlatforms';
...
const purchase: any = {
  ...
  user_data: { fp_client_id: 'fp.1.x', ip_address: '41.84.203.125', user_agent: 'UA', hashed_email: hashEmail(' Buyer@Example.com '), hashed_phone: hashPhone('0772 123 456') },
...
describe('advertising platforms: request builders', () => {
  ...
  it('Meta: Purchase to the dataset with hashed ids, IP/UA, value and order id; the token only in the URL', () => {
    ...
    expect(r.url).toBe('https://graph.facebook.com/v21.0/1234567890123/events?access_token=TOKEN_x');
