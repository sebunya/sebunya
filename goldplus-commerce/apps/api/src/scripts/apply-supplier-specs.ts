 ...
 */
interface SpecFile { products: Array<{ sku: string; title?: string; shortDescription?: string; longDescription?: string; specs: Array<{ name: string; value: string; unit?: string; isVerified?: boolean }> }> }
...
async function main(): Promise<void> {
  ...
  const dataFile = String(process.env.DATA_FILE ?? '').trim() || join(__dirname, 'data', 'supplier-specs-2026-09.json');
  const data = JSON.parse(readFileSync(dataFile, 'utf8')) as SpecFile;
  ...
  for (const entry of data.products) {
    ...
    console.log(`  ${entry.sku}: "${row.name}" → "${entry.title ?? row.name}" (+${entry.specs.length} specs)`);
    if (dryRun) continue;
    const result = await uc.execute({
      productId: String(row.id), name: entry.title, shortDescription: entry.shortDescription, longDescription: entry.longDescription,
      specs: entry.specs.map((s) => ({ ...s, isVerified: s.isVerified !== false })),
    });
    if (!result.ok) { console.log(`    FAILED: ${result.message}`); continue; }
    await audit.execute({ actorId, action: 'PRODUCT_LISTING_UPDATED', entity: 'product', entityId: String(row.id), newState: { ...result.changed, source: dataFile.split('/').pop() } });
    applied += 1;
  }
