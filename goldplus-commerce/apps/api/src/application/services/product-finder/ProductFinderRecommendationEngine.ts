...
export class ProductFinderRecommendationEngine {
  public static evaluate(
  ...
  ): { recommendedProducts: RecommendationResult[]; fallbackCategories: string[] } {
    ...
    const scoredProducts = eligibleProducts
      ...
      .map((p) => {
        ...
        // Category matching
        if (categoryFilter && p.categoryName && p.categoryName.toLowerCase().includes(categoryFilter.toLowerCase())) {
          score += 50;
        ...
        if (problemFit) {
          ...
          if (
            (problemLower.includes('fast charging') && lowerFeatures.some(f => f.includes('fast') || f.includes('quick'))) ||
            (problemLower.includes('travel') && lowerFeatures.some(f => f.includes('compact') || f.includes('portable') || f.includes('travel'))) ||
            (problemLower.includes('school') && lowerFeatures.some(f => f.includes('durable') || f.includes('student'))) ||
            (problemLower.includes('photos') && lowerFeatures.some(f => f.includes('high capacity') || f.includes('gb')))
          ) {
            score += 20;
        ...
        if (priorityFit) {
          ...
          } else if (priorityLower.includes('premium') && p.priceUgx >= 150000) {
            score += 15;
        ...
        if (budgetRange) {
          ...
          } else if (budgetLower.includes('mid') && p.priceUgx > 50000 && p.priceUgx <= 150000) {
            score += 20;
    ...
    return {
      recommendedProducts: topMatches.map((match): RecommendationResult => ({
        ...
        productUrl: `/products/${match.product.productId}`,
        matchScore: match.score,
        reasons: match.reasons.length > 0 ? Array.from(new Set(match.reasons)) : ['A great overall match'],
  ...
  private static parseAnswer(answer: string | string[] | undefined): string | null {
