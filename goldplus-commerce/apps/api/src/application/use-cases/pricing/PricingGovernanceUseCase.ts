...
import { PromotionStatus, PromotionVersionDraft, assertActivationWindow, canTransitionPromotion, normalizeCouponCode, validatePromotionVersion } from '../../../domain/pricing/Pricing';
...
export class PricingGovernanceUseCase {
  constructor(private readonly repo: IPricingRepository, private readonly audit: CreateAuditLogUseCase) {}
  ...
  async transition(input: { definitionId: string; versionId: string; expectedRevision: number; to: PromotionStatus; actorId: string; reason: string; now?: Date }) {
    ...
    await this.audit.execute({ actorId: input.actorId, action: `PRICING_PROMOTION_${input.to}`, entity: 'promotion_definition', entityId: input.definitionId, previousState: { status: version.status }, newState: { status: input.to, versionId: input.versionId, reason: input.reason.trim() } });
    return result;
  }
}
