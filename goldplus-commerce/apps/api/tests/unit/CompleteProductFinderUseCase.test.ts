...
describe('CompleteProductFinderUseCase', () => {
  it('generates recommendations and updates preferences safely', async () => {
    ...
    const mockCatalog = {
      findEligibleProducts: vi.fn().mockResolvedValue([
        { productId: 'p1', stockStatus: 'in_stock', features: ['fast'], categoryName: 'Power', priceUgx: 100 }
      ])
    } as any;
    ...
    const uc = new CompleteProductFinderUseCase(mockRepo, mockCatalog, mockMeasurement, mockPreference);
    const res = await uc.execute({ sessionId: 'sess-1' });
    ...
    expect(mockPreference.updateProductInterestsFromFinder).toHaveBeenCalledWith('u1', ['Power']);
  });
});
