...
/**
 * Same-origin relay for hero telemetry (Task 3, 2026-08-07).
 * One path, POST only, small body — an allowlist, not a forwarder.
 */
const API_BASE = (
...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    const res = await fetch(`${API_BASE}/hero/events`, { method: "POST", headers, body: raw, signal: AbortSignal.timeout(5000) });
