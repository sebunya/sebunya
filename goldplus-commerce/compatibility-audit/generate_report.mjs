...
write('network_failures.json', { run_id: RUN_ID, count: networkFailures.length, commerce_impact: networkFailures.filter((n) => n.impact === 'COMMERCE').length, optional_third_party: networkFailures.filter((n) => n.impact === 'OPTIONAL_THIRD_PARTY').length, failures: networkFailures });
...
const summary = { headline, journeys_passed: journeysPassed, journeys_failed: journeysFailed, tests_total: tests.length, tests_failed: byStatus('failed') + byStatus('timedOut'), console_errors: consoleErrors.length, network_failures: networkFailures.filter((n) => n.impact !== 'OPTIONAL_THIRD_PARTY').length, a11y_serious: a11y.reduce((a, r) => a + (r.serious ?? 0), 0), a11y_total: a11y.reduce((a, r) => a + (r.total ?? 0), 0), p0, p1, p2, p3, visual_regressions: rec('visual').filter((v) => v.status === 'DIFF').length, pwa_classification: pwaClass, data_usage: usageByJourney };
