...
const EXPECTED_FAILURE_TEST = /offline|interruption|storage loss|eviction|dropped connection|back \/ forward/i; // these tests cut the connection or cancel navigations on purpose
const consoleClass = (text) => (/beacon|telemetry|cloudflareinsights|respondWith|challenge|ERR_INTERNET_DISCONNECTED|Load failed|ERR_FAILED/i.test(text) ? 'ANALYTICS_OR_EXPECTED' : 'APPLICATION');
const consoleErrors = cn.flatMap((r) => (r.console ?? []).map((c) => ({ project: r.project, test: r.test, ...c, expected: EXPECTED_FAILURE_TEST.test(r.test), class: consoleClass(c.text) })));
const networkFailures = cn.flatMap((r) => (r.network ?? []).map((n) => ({ project: r.project, test: r.test, ...n, expected: EXPECTED_FAILURE_TEST.test(r.test) })));
const commerceNetworkFailures = networkFailures.filter((x) => x.impact === 'COMMERCE' && !x.expected);
const uniqueCommerce = new Map(); for (const n of commerceNetworkFailures) { const k = `${n.project}|${n.kind}|${n.url}`; if (!uniqueCommerce.has(k)) uniqueCommerce.set(k, n); }
for (const n of uniqueCommerce.values()) defects.push({ severity: 'P1', source: 'network', project: n.project, title: `first-party ${n.kind} on a commerce resource (${n.test.slice(0, 40)})`, detail: `${n.url} ${n.error ?? ''}` });
...
write('console_errors.json', { run_id: RUN_ID, count: consoleErrors.length, application_unexpected: consoleErrors.filter((c) => c.class === 'APPLICATION' && !c.expected).length, errors: consoleErrors });
write('network_failures.json', { run_id: RUN_ID, count: networkFailures.length, commerce_impact_unexpected: commerceNetworkFailures.length, commerce_impact: networkFailures.filter((n) => n.impact === 'COMMERCE').length, optional_third_party: networkFailures.filter((n) => n.impact === 'OPTIONAL_THIRD_PARTY').length, first_party_analytics: networkFailures.filter((n) => n.impact === 'FIRST_PARTY_ANALYTICS').length, failures: networkFailures });
...
const summary = { headline, journeys_passed: journeysPassed, journeys_failed: journeysFailed, journeys_blocked_by_edge: journeysBlocked, tests_blocked_by_edge: byStatus('blocked_by_edge'), tests_total: tests.length, tests_failed: byStatus('failed') + byStatus('timedOut'), console_errors: consoleErrors.filter((c) => c.class === 'APPLICATION' && !c.expected).length, console_errors_all: consoleErrors.length, network_failures: commerceNetworkFailures.length, network_failures_all: networkFailures.length, a11y_serious: a11y.reduce((a, r) => a + (r.serious ?? 0), 0), a11y_total: a11y.reduce((a, r) => a + (r.total ?? 0), 0), p0, p1, p2, p3, visual_regressions: rec('visual').filter((v) => v.status === 'DIFF').length, pwa_classification: pwaClass, data_usage: usageByJourney };
...
eng.push('', '## Console and network', '', `- console errors: ${consoleErrors.filter((c) => c.class === 'APPLICATION' && !c.expected).length} unexpected application errors of ${consoleErrors.length} captured (the rest: analytics beacons, Cloudflare challenge scripts, and tests that cut the connection on purpose); network failures: ${commerceNetworkFailures.length} commerce-impacting outside the interruption tests, of ${networkFailures.length} captured`, ...consoleErrors.filter((c) => c.class === 'APPLICATION' && !c.expected).slice(0, 10).map((c) => `  - ${c.project}: ${c.kind} ${c.text}`), ...commerceNetworkFailures.slice(0, 10).map((n) => `  - ${n.project}: ${n.kind} ${n.url} ${n.error ?? ''}`));
