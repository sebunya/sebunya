...
# Its own results file: the reporter names the file after COMPAT_PASS, and a
# second invocation under the same name replaced the journey results above, so
# the report counted 0 journeys and lost the failure details (2026-09-13).
COMPAT_PASS="${COMPAT_PASS}-network" pw network --project 'chromium:small_low_end_android' --grep 'slow_mobile|search under high latency'
finish_reports
exit 0
