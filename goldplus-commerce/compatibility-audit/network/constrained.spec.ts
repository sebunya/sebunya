...
test.describe('constrained profiles', () => {
  ...
  test('search under high latency: typing fast, out-of-order responses, then a dropped connection', async ({ page, gp }) => {
    ...
    await applyProfile(page, { network: 'high_latency', cpu: gp.cpu ?? 'reference' });
    await page.goto('/', { waitUntil: 'domcontentloaded' });
    ...
    await box.pressSequentially('ger', { delay: 40 });
    ...
    // Drop the connection while typing more: the sheet must not claim "no match" for a network failure.
    await page.context().setOffline(true);
    await box.pressSequentially('s', { delay: 40 });
    await page.waitForTimeout(1500);
    const textOffline = shown ? (await sheet.innerText().catch(() => '')).slice(0, 200) : '';
    await page.context().setOffline(false);
    const claimsNoMatchWhileOffline = /no match|no results/i.test(textOffline) && !/couldn.t|offline|connection|try again|unavailable/i.test(textOffline);
    ...
    await expectRenderedPage(page);
    gp.report('search_constraint', { suggestions_shown_online: shown, sheet_online: textOnline, sheet_offline: textOffline, network_failure_masquerades_as_empty: claimsNoMatchWhileOffline, status: claimsNoMatchWhileOffline ? 'FINDING' : 'PASS', severity: claimsNoMatchWhileOffline ? 'P2' : null, note: claimsNoMatchWhileOffline ? 'A dropped connection during suggestion fetch is rendered as "No match" (GpNav.astro catch → renderHits(q, [])). The shopper reads an empty catalogue instead of a connection problem.' : null });
  });
